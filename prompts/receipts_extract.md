...
- `category`: best-fit expense bucket for this purchase **only when clearly supported by the receipt**. Prefer one of: `cogs_parts`, `tools_equipment`, `shipping` when those clearly apply. If the receipt does not make the expense category clear, set `category` to `null` and include `"category"` in `Fields_not_found`—do not guess.
