...
- now i need to package everything we've done in a way the professor can grade and install dependencies, rerun our pipeline and review prompts and outputs. we need them to be able to tell we took this step by step. Everything we've done needs to go in a folder named hw1. it MUST include the prompts in the .md files, the requirements.txt file, readme.md with install steps and the pipeline.html if there's a way to chain scripts in order so they make sense to a grader, do that.
- oh WAIT be sure to include all scripts from problems 2 - 8, if i didn't say that
